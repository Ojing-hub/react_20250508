...
const Port = () => {
    ...
    // horizentalRef라는 변수를 생성하고 useRef 훅으로 초기화하라는 명령어
    const sectionsRef = useRef([]);
    // sectionsRef라는 변수를 생성하고 useRef 훅으로 배열값으로 초기화하라는 명령어

    // useRef() : react에서 변하는 값을 저장하는 명령어
    // useRef(null): 빈 상태의 값을 저장하라는 명령어 > 문자값과 숫자값이 모두 가능(숫자도 문자로 치환해서 저장)
    // useRef([]): 배열값을 저장하라는 명령어
    // useRef(0): 숫자값을 저장하라는 명령어

    useEffect(()=>{
        gsap.registerPlugin(ScrollTrigger);

        const horizontal = horizontalRef.current;
        // .current: useRef 객체의 속성(실제 참조하고 있는 값)
        ...
        let scrollTween = gsap.to(sections,{
            ...
            // 왼쪽으로 이동하는 이동 위치값 지정
            ease:"none",
            // 속도를 일정하게 하기 위해 none을 지정

            scrollTrigger:{
                // scrollTrigger: 를 시작하겠다는 의미
                trigger : horizontal,
                start : "top 50px",
                end : "+=3000",
                pin : true,
